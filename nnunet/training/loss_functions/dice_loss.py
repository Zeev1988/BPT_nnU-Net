...
def get_soft_tp_fp_fn_tn(net_output, gt, axes=None, mask=None, square=False):
    """
    net_output must be (b, c, x, y(, z)))
    gt must be a label map (shape (b, 1, x, y(, z)) OR shape (b, x, y(, z))) or one hot encoding (b, c, x, y(, z))
    if mask is provided it must have shape (b, 1, x, y(, z)))
    ...
    with torch.no_grad():
        ...
        if all([i == j for i, j in zip(net_output.shape, gt.shape)]):
            ...
            y_onehot = gt.expand(-1, 2, -1, -1, -1).clone()
            y_onehot[:, 0] = (1-gt[:][0]).float()
...
class SoftLabelDiceLoss(nn.Module):
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        #tp, fp, fn, _ = get_soft_tp_fp_fn_tn(x, y, axes, loss_mask, False)
        alpha1 = 0.5 if int(self.data_dict['keys'][0].rsplit('_')[-1]) > 65 else 1
        alpha2 = 0.5 if int(self.data_dict['keys'][1].rsplit('_')[-1]) > 65 else 1
        dice1 = ((x[0][1] * y[0]).sum()) / ((0.5 * x[0][1].sum()) + (0.5 * y[0].sum()))
        dice2 = ((x[1][1] * y[1]).sum()) / ((0.5 * x[1][1].sum()) + (0.5 * y[1].sum()))
        dc = (dice1+dice2)/2

        return ((alpha1*(1-dice1)) + (alpha2*(1-dice2)))/2
...
class SoftLabelDiceLossSquared(nn.Module):
    def __init__(self, apply_nonlin=None, batch_dice=False, do_bg=True, smooth=1.):
        """
        squares the terms in the denominator as proposed by Milletari et al.
        """
        super(SoftLabelDiceLossSquared, self).__init__()
...
class SDC_and_CE_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, aggregate="sum", square_dice=False, weight_ce=1, weight_dice=1,
                 log_dice=False, ignore_label=None):
        """
        CAREFUL. Weights for CE and Dice do not need to sum to one. You can set whatever you want.
        ...
        :param weight_dice:
        """
        super(SDC_and_CE_loss, self).__init__()
        if ignore_label is not None:
            assert not square_dice, 'not implemented'
            ce_kwargs['reduction'] = 'none'
        self.log_dice = log_dice
        ...
        if not square_dice:
            self.dc = SoftLabelDiceLoss(apply_nonlin=softmax_helper, **soft_dice_kwargs)
        else:
            self.dc = SoftLabelDiceLossSquared(apply_nonlin=softmax_helper, **soft_dice_kwargs)
    ...
    def forward(self, net_output, target):
        ...
        self.dc.set_data_dict(self.data_dict)
        dc_loss = self.dc(net_output, target, loss_mask=mask) if self.weight_dice != 0 else 0
        if self.log_dice:
            dc_loss = -torch.log(-dc_loss)
        ...
        if self.aggregate == "sum":
            result = dc_loss #self.weight_ce * ce_loss + self.weight_dice * dc_loss
        else:
            raise NotImplementedError("nah son") # reserved for other stuff (later)
        return result
