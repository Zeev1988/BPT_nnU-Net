...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False, numOfEpochs=None):
        """
        ...
        if numOfEpochs:
            self.max_num_epochs = numOfEpochs
        else:
            self.max_num_epochs = 5 #Added by us
    ...
    def run_training(self):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            with open(r"D:\users\zeevh\nnUNet\nnUNet-1-New\nnunet\logs.txt", 'a') as log_file:
                timestamp = time()
                dt_object = datetime.fromtimestamp(timestamp)
                log_file.write("%s epoch: %d \n" % (dt_object, self.epoch))  ##print_epoch_to_logfile
            ...
            self.print_to_log_file("\nepoch: ", self.epoch)
            with open(r"D:\users\zeevh\nnUNet\nnUNet-1-New\nnunet\logs.txt", 'a') as log_file:
                log_file.write("This epoch took %f s\n" % (epoch_end_time - epoch_start_time))  ##print_epoch_to_logfile
    ...
    def load_pretrained_weights(self, fname):
        ...
        for key, _ in model_dict.items():
            if ('conv_blocks' in key):
                ...
                if (key in pretrained_dict) and (model_dict[key].shape == pretrained_dict[key].shape):
                    #print("key", key, " pretrained dict", pretrained_dict)
                    #print("(model_dict[key].shape", model_dict[key].shape, " pretrained_dict[key].shape", pretrained_dict[key].shape)
                    continue
                else:
                    fine_tune = False
                    break
        ...
        if fine_tune:
            ...
            for i, param in enumerate(self.network.parameters()):
                if 'seg' in keys[i] or 'tu' in keys[i]:
