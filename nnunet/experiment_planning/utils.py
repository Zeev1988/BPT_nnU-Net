...
def get_file_real_path(base_folder_splitted, shortcut):
    if f'{shortcut}.lnk' in os.listdir(base_folder_splitted):
        shell = win32com.client.Dispatch("WScript.Shell")
        real_path = shell.CreateShortCut(os.path.join(base_folder_splitted, f'{shortcut}.lnk'))
        return real_path.TargetPath
    return shortcut
...
def create_lists_from_splitted_dataset(base_folder_splitted):
    ...
    for tr in training_files:
        ...
        for mod in range(num_modalities):
            cur_pat.append(join(base_folder_splitted, get_file_real_path(base_folder_splitted, "imagesTr"),
                                os.path.basename(tr['image'])[:-7] + "_%04.0d.nii.gz" % mod))
        cur_pat.append(join(base_folder_splitted, get_file_real_path(base_folder_splitted, "labelsTr"), os.path.basename(tr['label'])))
