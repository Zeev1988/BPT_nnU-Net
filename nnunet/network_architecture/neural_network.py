...
class SegmentationNetwork(NeuralNetwork):
        ...
        for x in steps[0]:
            ...
            for y in steps[1]:
                ...
                for z in steps[2]:
                    ...
                    predicted_patch, all_preds = self._internal_maybe_mirror_and_pred_3D(
    ...
    def _internal_predict_3D_3Dconv(self, x: np.ndarray, min_size: Tuple[int, ...], do_mirroring: bool,
        ...
        predicted_probabilities, all_preds = self._internal_maybe_mirror_and_pred_3D(data[None], mirror_axes, do_mirroring,
                                                                          None)[0]
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
        ...
        for m in range(mirror_idx):
            ...
            if m == 5 and (0 in mirror_axes) and (2 in mirror_axes):
                ...
                pred = torch.flip(pred, (4, 2))

            if m == 6 and (0 in mirror_axes) and (1 in mirror_axes):
                pred = self.inference_apply_nonlin(self(torch.flip(x, (3, 2))))
                result_torch += 1 / num_results * torch.flip(pred, (3, 2))
                pred = torch.flip(pred, (3, 2))

            if m == 7 and (0 in mirror_axes) and (1 in mirror_axes) and (2 in mirror_axes):
                pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3, 2))))
                result_torch += 1 / num_results * torch.flip(pred, (4, 3, 2))
                pred = torch.flip(pred, (4, 3, 2))

            all_pred.append(pred)
        if mult is not None:
            result_torch[:, :] *= mult

        return result_torch, all_pred
