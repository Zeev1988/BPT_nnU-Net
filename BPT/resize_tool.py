...
def resize(subject: tuple, output_shape: List[int], output_dir: str, shrink: bool):
    ...
    label_orig = None
    label_direct = None
    modality_list = ['FLAIR','Label', 'T1', 'T1C', 'T2']
    ...
    for modality in modality_list:
        if modules[modality]:
            ...
            # trim to minimum bounds
            # array = zero_trim_ndarray(array)
            #
            # # pad if input size is smaller than output size
            # z_pad = (output_shape[2] - array.shape[0]) / 2 if (output_shape[2] - array.shape[0]) > 0 else 0
            # y_pad = (output_shape[1] - array.shape[1]) / 2 if (output_shape[1] - array.shape[1]) > 0 else 0
            # x_pad = (output_shape[0] - array.shape[2]) / 2 if (output_shape[0] - array.shape[2]) > 0 else 0
            # array = np.pad(array,
            #                ((int(z_pad), int(z_pad) + (not z_pad.is_integer())),
            #                 (int(y_pad), int(y_pad) + (not y_pad.is_integer())),
            #                 (int(x_pad), int(x_pad) + (not x_pad.is_integer()))), 'constant')

            # zoom out if input shape is still to big
