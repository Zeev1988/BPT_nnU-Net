...
def get_params_fname(fold):
    return os.path.join(bpt_params.folder_with_parameter_files, f"{fold}.model")
...
def maybe_download_parameters(fold=0, force_overwrite=False):
    ...
    if not os.path.isdir(bpt_params.folder_with_parameter_files):
        Path(bpt_params.folder_with_parameter_files).mkdir(parents=True, exist_ok=True)
